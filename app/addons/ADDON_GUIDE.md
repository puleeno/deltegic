...
Create a folder in `~/.local/share/nexdl/addons/` (or the addon directory configured in Settings):
...
Your `__init__.py` must define a class that inherits from `nexdl.Addon`:
...
```python
item = nexdl.DownloadItem(url, title="", output_path="", filename="")
...
Cookies are managed per-account in NexDL. When an addon is invoked for a task
that has an account assigned, the account's cookies are automatically injected
into `ctx.http`.

To import cookies:
1. Open **Accounts** in NexDL
